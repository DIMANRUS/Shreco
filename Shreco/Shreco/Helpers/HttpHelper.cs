...
internal class HttpHelper : IDisposable
{
    ...
    {
#if DEBUG
        BaseAddress = new Uri("http://192.168.19.209:5000")
#else
    ...
    public async Task<HttpResponseMessage> PutRequest<T>(string url, T model, string token = "")
    {
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token == "" ? await UserDataStore.Get(DatasNames.Token) : token);
        string json = JsonSerializer.Serialize(model);
        return await _httpClient.PutAsync(url, new StringContent(json, Encoding.UTF8, "application/json"));
    }
