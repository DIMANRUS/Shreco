        ...
        AuthCommand = new AsyncCommand(async () =>
        {
            ...
            if (!string.IsNullOrEmpty(Email))
            {
                try
                {
                    ...
                    HttpResponseMessage httpResponse = await httpHelper.GetRequest($"/User/CheckExistUserByEmail?email={user.Email}");
                    if (httpResponse.StatusCode == HttpStatusCode.OK || _isRegistration)
                    {
                        httpResponse = await httpHelper.GetRequest($"/Auth/SendCode/{user.Email}");
                        if (httpResponse.IsSuccessStatusCode)
                        {
                            ...
                            if (!string.IsNullOrEmpty(userCode))
                            {
                                ...
                                if (_isRegistration)
                                    response = await httpHelper.PostRequest($"/Auth/Register/{userCode}", user, await httpResponse.Content.ReadAsStringAsync());
                                else
                                    response = await httpHelper.GetRequest($"/Auth?email={user.Email}&code={userCode}", await httpResponse.Content.ReadAsStringAsync());
